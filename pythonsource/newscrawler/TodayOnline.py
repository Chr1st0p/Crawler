...
def parseHtml(iters, match, i):
    ...
    if matchlink:
        ...
        year, month = TodayOnlineDateParse(date[0])
    ...
    data = (iters, title, link, category, keyword, year, month, content)
...
def createTable():
    sqlCreateTable = "CREATE TABLE IF NOT EXISTS todayonlinedata (" \
...
def insertData(data):
    cursor = TodayNews.cnn.cursor()
    sql_insert = 'INSERT INTO todayonlinedata (page, title, link, category,keyword,postyear,postmonth, content) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)'
    try:
